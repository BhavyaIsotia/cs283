 ...
 */
 int start_server(char *ifaces, int port, int is_threaded){
    ...
    printf("DEBUG: Server started on %s:%d\n", ifaces, port);
    return rc;
 ...
 */
 int process_cli_requests(int svr_socket){
    ...
    while(1){
        ...
        if (rc == OK_EXIT) {
            printf("Stopping server as requested by client...\n");
            break;
        }
    }
 ...
 */
 int exec_client_requests(int cli_socket) {
    ...
    while (1) {
        ...
        if (parse_pipeline(io_buff, &cmd_list) != OK) {
 ...
 */
 int send_message_string(int cli_socket, char *buff){
    ...
    if (sent_len < 0) return ERR_RDSH_COMMUNICATION;
 ...
 */
 int rsh_execute_pipeline(int cli_sock, command_list_t *clist) {
    ...
    for (int i = 0; i < clist->num; i++) {
        ...
        if (i == clist->num - 1) {
            exit_code = WIFEXITED(status) ? WEXITSTATUS(status) : ERR_RDSH_COMMUNICATION;
        }
    }
    return exit_code;
}
