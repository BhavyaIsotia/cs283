...
{
    // Check if the file descriptor is valid and pointer is non-null
    if (fd < 0 || s == NULL)
    {
        return ERR_DB_FILE;  // Invalid input parameters, return error
    }

    student_t student;
    ssize_t bytesRead;

    // Read records from the file and compare with the id
    while ((bytesRead = read(fd, &student, STUDENT_RECORD_SIZE)) > 0)
    {
        // If the student id matches, copy the data into *s
        if (student.id == id)
        {
            memcpy(s, &student, STUDENT_RECORD_SIZE);
            return NO_ERROR;  // Success, student found
        }
    }
    ...
    if (bytesRead == -1)
    {
        return ERR_DB_FILE;  // File read error
    }
...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    // Ensure the file is at least 64,000,000 bytes using lseek()
    lseek(fd, 6399999, SEEK_SET);  // Move to the last byte of the expected size
    write(fd, "\0", 1);  // Write a single null byte to expand file size
    ...
    lseek(fd, offset, SEEK_SET);  // Move to the correct student position

    student_t existing_student;
    read(fd, &existing_student, sizeof(student_t));
    ...
    write(fd, &new_student, sizeof(student_t));
    fsync(fd);
...
int print_db(int fd)
{
    ...
    while ((bytesRead = read(fd, &student, sizeof(student_t))) > 0) {
        if (memcmp(&student, &EMPTY_STUDENT_RECORD, sizeof(student_t)) != 0) {
            ...
            printf("%d %s %s %.2f\n", student.id, student.fname, student.lname, gpa);
        }
    }
...
{
    // Check if student pointer is NULL or student ID is zero
    if (s == NULL || s->id == 0) {
    ...
    // Calculate GPA as a float (since it's stored as an integer, e.g., 3400 for 3.40)
...
int compress_db(int fd)
{
    ...
    tmp_fd = open(TMP_DB_FILE, O_RDWR | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR);

    if (tmp_fd == -1) {
        printf(M_ERR_DB_OPEN);  // Error creating temporary file
        return ERR_DB_FILE;
    }

    // Seek to the beginning of the original database file
    if (lseek(fd, 0, SEEK_SET) == -1) {
        printf(M_ERR_DB_READ);  // Error seeking database file
        close(tmp_fd);
        return ERR_DB_FILE;
    }

    // Read through the database and write valid records to the temp file
    while ((bytesRead = read(fd, &student, sizeof(student_t))) > 0) {
        // Check if the record is not empty (not zeroed-out)
        if (memcmp(&student, (const void *)&EMPTY_STUDENT_RECORD, sizeof(student_t)) != 0) {
            // Write valid student record to the temporary file
            if (write(tmp_fd, &student, sizeof(student_t)) == -1) {
                printf(M_ERR_DB_WRITE);  // Error writing to the temp file
                close(tmp_fd);
                return ERR_DB_FILE;
            }
        }
    ...
    // If read failed
    if (bytesRead == -1) {
        printf(M_ERR_DB_READ);  // Error reading from the original database
        close(tmp_fd);
        return ERR_DB_FILE;
    }
    ...
    // Rename the temporary file to replace the original database file
    if (rename(TMP_DB_FILE, DB_FILE) == -1) {
        printf(M_ERR_DB_CREATE);  // Error renaming the temporary file
        return ERR_DB_FILE;
    }

    // Reopen the database file (now compressed)
    fd = open(DB_FILE, O_RDWR);

    if (fd == -1) {
        printf(M_ERR_DB_OPEN);  // Error opening the compressed database
        return ERR_DB_FILE;
    }

    // Return the file descriptor for the compressed database
